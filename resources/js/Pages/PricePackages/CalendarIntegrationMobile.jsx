    ...
    const fetchDropdownAvailability = useCallback(async () => {
        ...
        try {
            ...
            for (let i = 0; i < datesToFetch.length; i += batchSize) {
                ...
                const nextSlots = Object.fromEntries(results);
                timeSlotsRef.current = {
                    ...timeSlotsRef.current,
                    ...nextSlots,
                };
...
//     const mapAvailableSlots = useCallback((slots = []) =>
//         slots
//             .filter((slot) => slot.status === "available")
//             .map((slot) => {
//                 const startTime = slot.start_time;
...
//     const fetchDropdownAvailability = useCallback(async () => {
//         if (!price?.id || allDates.length === 0 || availabilityLoadingRef.current) {
//             return;
//         }
...
//     // Fetch time slots for a specific date.
//     const fetchSlotsForDate = useCallback(async (dateKey, force = false) => {
//         if (!dateKey || !price?.id || isPastDate(dateKey)) return;
