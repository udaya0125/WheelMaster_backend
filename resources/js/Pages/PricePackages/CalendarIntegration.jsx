...
const CalendarIntegration = ({ price }) => {
    ...
    // ─── NEW: Fetch ALL availability upfront (next 90 days) ───────────────
    const fetchAllAvailability = useCallback(async () => {
        ...
        for (let i = 0; i < datesToFetch.length; i += batchSize) {
            ...
            const promises = batch.map(async (date) => {
                ...
                try {
                    ...
                    if (response.data.success) {
                        ...
                        allAvailability[dateKey] = hasAvailable
                            ? "available"
                            : "unavailable";
                        ...
                        if (hasAvailable) {
                            ...
                            allTimeSlots[dateKey] = availableTimes;
                        }

                        // Also store full slot data for the selected date
                        ...
                        allAvailability[dateKey] = "unavailable";
                    }
                } catch {
                    allAvailability[dateKey] = "unavailable";
                }
            ...
            setDayAvailability(prev => ({ ...prev, ...allAvailability }));
            setTimeSlots(prev => ({ ...prev, ...allTimeSlots }));
        }

        setInitialLoading(false);
    }, [price?.id, selectedDate]);

    // ─── Initial load: fetch everything upfront ───────────────────────────
    useEffect(() => {
        fetchAllAvailability();
    }, [fetchAllAvailability]);

    // ─── Only fetch detailed slots for selected date if not already loaded ──
    useEffect(() => {
        const fetchDetailedSlots = async () => {
            const dateKey = formatDateKey(selectedDate);
            
            // Skip if we already have detailed data for this date
            if (allSlotsData[dateKey]) {
                const slots = allSlotsData[dateKey];
                const booked = slots
                    .filter(
                        (slot) =>
                            slot.status === "reserved" ||
        ...
        if (selectedDate && price?.id && !initialLoading) {
            fetchDetailedSlots();
        }
    }, [price?.id, selectedDate, initialLoading, allSlotsData]);

    // ─── Keep month availability in sync (but now we already have most data) ──
    const fetchMonthAvailability = useCallback(
        async (monthDate) => {
            if (!price?.id || initialLoading) return;
        ...
        [price?.id, dayAvailability, initialLoading]
    );
    ...
    }, [currentMonth, fetchMonthAvailability, initialLoading]);
    ...
    const findNextAvailableDates = async () => {
        try {
            ...
            for (let i = 1; i <= 30; i++) {
                ...
                // Check if we already have the availability data
                if (dayAvailability[dateKey] === "available") {
                    availableDates.push(new Date(nextDate));
                } else if (!dayAvailability[dateKey]) {
