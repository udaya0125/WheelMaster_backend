...
const TestCalendarIntegrationMobile = ({ price }) => {
    ...
    const [showCheckoutOptions, setShowCheckoutOptions] = useState(false);
    const [formUnlocked, setFormUnlocked] = useState(false);
    // Remembers WHICH checkout option was picked ("guest" vs "user") — a
    // logged-in visitor can still choose Guest Checkout, so the
    // saved-profile banner/autofill below is only shown when
    // checkoutMode === "user".
