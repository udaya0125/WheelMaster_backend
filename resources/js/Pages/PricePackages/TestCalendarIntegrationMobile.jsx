...
const TestCalendarIntegrationMobile = ({ price }) => {
    ...
    // Selected location objects for validation (mirrors CalendarIntegrationMobile)
    ...
    // Build the full combined address string: "12 oak street, Ranceby Avenue, Mandurah, Western Australia 6210"
    ...
    const handleFormChange = (e) => {
        ...
        if (name === "address") {
            if (value === MEETPOINT_AREA) {
                // Selecting Meetpoint: auto-fill pickup + dropoff
                setBookingForm((prev) => ({
                    ...prev,
                    address: value,
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    onChange={handleFormChange}
                                                    required
                                                    className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.pickup_home_address ? "border-red-500" : "border-gray-300"}`}
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    onChange={handleFormChange}
                                                    required
                                                    className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.dropoff_home_address ? "border-red-500" : "border-gray-300"}`}
                                                    placeholder="e.g. 12 Oak Street, Unit 4"
                                                />
                                            </div>
...
//             if (
//                 !selectedLocations[field] ||
//                 !locationMatchesTypedAddress(selectedLocations[field], bookingForm[field])
//             ) {
//                 locationErrors[field] = `Please choose a service-area suggestion for the ${label}, then add house/unit details if needed.`;
//             }
//         });
//         return locationErrors;
//     };
...
//     const setPickupSameAsAddress = () => {
//         if (bookingForm.address) {
//             setBookingForm((prev) => ({ ...prev, pickup_location: prev.address }));
//             if (formErrors.pickup_location) {
//                 setFormErrors((prev) => ({ ...prev, pickup_location: "" }));
//             }
//             toast.success("Pickup location set to address");
...
//             )
//         ) {
//             setBookingForm((prev) => ({ ...prev, dropoff_location: prev.pickup_location }));
//             setSelectedLocations((prev) => ({
//                 ...prev,
//                 dropoff_location: prev.pickup_location,
//             }));
//             if (formErrors.dropoff_location) {
//                 setFormErrors((prev) => ({ ...prev, dropoff_location: "" }));
//             }
//             toast.success("Dropoff location set to pickup location");
//         } else {
...
//             const date = new Date(today);
//             date.setDate(today.getDate() + i);
//             const monthYear = date.toLocaleDateString("en-AU", { month: "long", year: "numeric" });
//             const formatted = date.toLocaleDateString("en-AU", {
...
//         if (minuteMatch) total += parseInt(minuteMatch[1]);
//         if (total === 0) {
//             const num = parseFloat((clean.match(/(\d+(?:\.\d+)?)/) || [])[1] || 0);
//             total = num < 10 ? Math.round(num * 60) : Math.round(num);
//         }
//         return total || 60;
//     };
...
//         setLoadingSlots(true);
//         try {
//             const response = await axios.get(route("test-packages.available-slots"), {
//                 params: {
//                     date: selectedDate,
//                     price_id: price.id,
//                     duration_minutes: parseDuration(price.duration),
//                 },
//             });
//             setAvailableTimeSlots(response.data.success ? response.data.available_slots : []);
//         } catch (error) {
//             console.error("Error fetching available slots:", error);
//             setAvailableTimeSlots([]);
//         } finally {
...
//     const checkTestAvailability = async () => {
//         if (!price) { toast.error("Price information not available"); return; }
...
//         try {
//             const formattedTime = formatTimeForApi(selectedTime);
//             const response = await axios.post(route("test-packages.check-availability"), {
//                 date: selectedDate,
//                 test_time: formattedTime,
//                 duration_minutes: parseDuration(price.duration),
//                 price_id: price.id,
...
//             if (error.response?.status === 422) {
//                 const errors = error.response.data.errors;
//                 const msg = errors?.test_time?.[0] || errors?.price_id?.[0] || "Please check the time format (HH:MM) and try again.";
//                 if (errors?.test_time) setTimeError(errors.test_time[0]);
//                 setAvailabilityMessage(`Validation error: ${msg}`);
//                 toast.error(msg);
//             } else {
...
//             ...prev,
//             [name]: value,
//             // Auto-fill pickup/dropoff when meetpoint is selected
...
//         }
//         if (!acceptTerms) {
//             setFormErrors({ terms: "Please accept the Terms & Conditions and Privacy Policy" });
//             toast.error("Please accept the Terms & Conditions and Privacy Policy");
//             return;
//         }

//         const requiredFields = ["user_name", "email", "phone", "address", "pickup_location", "dropoff_location"];
//         const newErrors = {};
//         requiredFields.forEach((field) => {
//             if (!bookingForm[field]?.trim()) {
//                 newErrors[field] = `${field.replace("_", " ")} is required`;
...
//             const response = await axios.post(route("test-packages.store"), bookingData);
//             toast.dismiss(submittingToast);
...
//                 setTimeError("");
//                 setAcceptTerms(false);
//                 setSelectedLocations({ pickup_location: null, dropoff_location: null });
//                 setBookingForm({
//                     user_name: "",
//                     email: "",
//                     phone: "",
...
//                     duration: 4000,
//                     style: { background: "#363636", color: "#fff" },
//                     success: { duration: 3000, style: { background: "#10b981", color: "#fff" }, iconTheme: { primary: "#fff", secondary: "#10b981" } },
//                     error: { duration: 4000, style: { background: "#ef4444", color: "#fff" }, iconTheme: { primary: "#fff", secondary: "#ef4444" } },
//                     loading: { duration: 5000, style: { background: "#3b82f6", color: "#fff" } },
//                 }}
//             />
...
//                 <div className="mb-6">
//                     <div className="flex items-center gap-3 mb-1">
//                         <h1 className="text-2xl font-bold text-gray-900 ml-6">Schedule Your Test Package</h1>
//                     </div>
//                     <p className="text-gray-500 text-sm ml-6">
//                         Choose your date and time. Operating hours: 7:00 AM - 6:00 PM
//                     </p>
//                 </div>
...
//                                         {dates.map((date, i) => (
//                                             <option key={i} value={date.value} disabled={isPastDate(date.value)} className="py-1">
...
//                         </div>
//                         {selectedDate && (
//                             <p className="text-sm text-gray-600 mt-3">Selected: {formatDisplayDate(selectedDate)}</p>
//                         )}
//                     </div>
...
//                         <p className="text-sm text-gray-500 mb-4">
//                             {selectedDate ? formatDisplayDate(selectedDate) : "Please select a date first"}
//                         </p>
...
//                                             <option key={index} value={slot.time}>{slot.formatted}</option>
//                                         ))}
//                                     </select>
//                                     <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 pointer-events-none" />
//                                 </div>
...
//                                     Test Start Time (24-hour format) *
//                                 </label>
//                                 <p className="text-xs text-gray-500 mb-2">Operating hours: 7:00 AM - 6:00 PM</p>
//                                 <div className="relative">
//                                     <Clock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 pointer-events-none z-10" />
//                                     <input
//                                         type="time"
...
//                             <button
//                                 onClick={checkTestAvailability}
//                                 disabled={!selectedTime || loading || !selectedDate}
//                                 className={`w-full py-3 px-4 rounded-xl font-semibold transition-all duration-200 text-sm ${
//                                     selectedTime && !loading && selectedDate
//                                         ? "bg-blue-600 hover:bg-blue-700 text-white"
//                                         : "bg-gray-200 text-gray-400 cursor-not-allowed"
...
//                         {alternativeTimes.length > 0 && (
//                             <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
//                                 <h4 className="font-medium text-blue-800 mb-2">Suggested Available Times:</h4>
//                                 <div className="flex flex-wrap gap-2">
//                                     {alternativeTimes.map((slot, index) => (
//                                         <button
//                                             key={index}
...
//                         {availabilityMessage && (
//                             <div className={`mt-6 p-4 rounded-xl ${isAvailable ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
//                                 <div className="flex items-start">
...
//                                         {!isAvailable && (
//                                             <div className="mt-2">
//                                                 <p>Phone: <a href="tel:0481488216" className="text-blue-600 underline hover:text-blue-800">0481488216</a></p>
//                                                 <p>Email: <a href="mailto:Wheelmaster@outlook.com.au" className="text-blue-600 underline hover:text-blue-800">Wheelmaster@outlook.com.au</a></p>
//                                             </div>
//                                         )}
//                                     </div>
//                                 </div>
...
//                                     <div className="relative">
//                                         <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
//                                         <input type="text" id="user_name" name="user_name" value={bookingForm.user_name} onChange={handleFormChange} required
//                                             className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.user_name ? "border-red-500" : "border-gray-300"}`}
//                                             placeholder="John Doe" />
//                                     </div>
//                                     {formErrors.user_name && <p className="mt-1 text-sm text-red-600">{formErrors.user_name}</p>}
//                                 </div>
...
//                                     <div className="relative">
//                                         <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
//                                         <input type="email" id="email" name="email" value={bookingForm.email} onChange={handleFormChange} required
//                                             className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.email ? "border-red-500" : "border-gray-300"}`}
//                                             placeholder="john@example.com" />
//                                     </div>
...
//                                     <div className="relative">
//                                         <Phone className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
//                                         <input type="tel" id="phone" name="phone" value={bookingForm.phone} onChange={handleFormChange} required
//                                             className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.phone ? "border-red-500" : "border-gray-300"}`}
//                                             placeholder="+61 4XX XXX XXX" />
//                                     </div>
...
//                                 {/* Area */}
//                                 <div>
//                                     <label htmlFor="address" className="block text-sm font-medium text-gray-700 mb-2">Area *</label>
//                                     <div className="relative">
//                                         <Home className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 pointer-events-none" />
//                                         <select id="address" name="address" value={bookingForm.address} onChange={handleFormChange} required
//                                             className={`w-full appearance-none pl-10 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition bg-white ${formErrors.address ? "border-red-500" : "border-gray-300"}`}>
...
//                                 {/* Test Location */}
//                                 <div>
//                                     <label htmlFor="test_location" className="block text-sm font-medium text-gray-700 mb-2">Test Location *</label>
//                                     <div className="relative">
//                                         <MapPinIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
//                                         <input type="text" id="test_location" name="test_location" value={bookingForm.test_location} onChange={handleFormChange} required
//                                             className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${formErrors.test_location ? "border-red-500" : "border-gray-300"}`}
//                                             placeholder="Enter test location" />
//                                     </div>
//                                     {formErrors.test_location && <p className="mt-1 text-sm text-red-600">{formErrors.test_location}</p>}
//                                 </div>
...
//                                     <textarea id="comment" name="comment" value={bookingForm.comment} onChange={handleFormChange} rows={6}
//                                         className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition resize-none"
//                                         placeholder="Any special requests, notes for your instructor, etc." />
//                                 </div>
...
//                                             }}
//                                             className="mt-1 h-5 w-5 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" required />
//                                         <label htmlFor="acceptTerms" className="text-sm text-gray-700">
//                                             I agree to the{" "}
//                                             <a href="https://wheelmasterdriving.com.au/terms" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 underline font-medium">Terms & Conditions</a>
//                                             {" "}and{" "}
//                                             <a href="https://wheelmasterdriving.com.au/policy" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 underline font-medium">Privacy Policy</a> *
//                                         </label>
//                                     </div>
...
//                     {/* ── Order Summary ──────────────────────────────────── */}
//                     <div>
//                         <div onClick={() => setShowOrderSummary(!showOrderSummary)} className="cursor-pointer select-none">
//                             <div className="flex items-center justify-between">
...
//                                     <ChevronDown className={`h-5 w-5 text-gray-400 transition-transform duration-300 ${showOrderSummary ? "rotate-180" : ""}`} />
//                                 </div>
//                             </div>
...
//                                     <div className="space-y-4">
//                                         <div>
//                                             <h3 className="font-semibold text-gray-900 mb-1 text-base">{price.description}</h3>
//                                             <p className="text-sm text-gray-600">Professional driving test preparation with certified instructors</p>
//                                         </div>
//                                         <div className="border-t pt-4">
//                                             <div className="flex justify-between text-sm mb-2">
