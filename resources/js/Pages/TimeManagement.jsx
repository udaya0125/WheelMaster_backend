...
const TimeManagement = () => {
    ...
    const calculateStats = (slots) => {
        ...
        setStats({ total, available, reserved, blocked,
                   default: default_count, custom: total - default_count });
    };
    ...
    const handleDateSelect = (date) => { if (date) setSelectedDate(date); };

    // ── Reset ──────────────────────────────────────────────────────────────────

    const handleResetToDefault = async () => {
        if (!confirm("Reset ALL slots for this date to the default 7:00 AM schedule?")) return;
    ...
    const handleTimeEditBlur     = (e, slot, index) => handleTimeEditSubmit(e, slot, index);
    const handleTimeEditKeyPress = (e, slot, index) => {
    ...
    const handleTimeEditSubmit = async (e, slot, index) => {
        ...
        if (!newTime) { cancelEditing(index); return; }
        ...
        if (!window.confirm(confirmMessage)) { cancelEditing(index); return; }
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const updatedSlots = response.data.slots.map((s) => ({
                    ...s,
                    isEditing:       false,
                    formatted_start: s.formatted_start || formatTime(s.start_time),
                }));
                ...
                if (response.data.custom_start_info) setCustomStartInfo(response.data.custom_start_info);
                if (response.data.current_start)     setCustomStartTime(response.data.current_start);
            } else {
                throw new Error(response.data.message || "Failed to update slots");
            }
        } catch (error) {
            console.error("Error updating time slots:", error);
            toast.error(error.response?.data?.message || error.message || "Failed to update time slots");
            cancelEditing(index);
        } finally {
            setLoading(false);
        }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 lg:col-span-1">
                            ...
                            <Calendar
                                ...
                                toYear={new Date().getFullYear() + 8}
                                formatters={{
                                    formatMonthCaption: (d) => d.toLocaleString("default", { month: "long" }),
                                    formatYearCaption:  (d) => d.getFullYear().toString(),
                                }}
                            />
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white rounded-lg shadow-sm p-6">
                                <div className="flex items-center justify-between mb-4">
                                    <h2 className="text-lg font-semibold text-gray-800">
                                        Time Slots for {formatDisplayDate(selectedDate)}
                                    </h2>
                                    <button
                                        onClick={handleResetToDefault}
                                        disabled={loading}
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                            {timeSlots.map((slot, index) => (
                                                ...
                                                >
                                                    ...
                                                    {customStartInfo && customStartInfo.index === index && (
                                                        <div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
                                                    ...
                                                    {slot.isEditing ? (
                                                        <div onClick={(e) => e.stopPropagation()} className="w-full">
                                                            <input
                                                                ...
                                                                onBlur={(e)    => handleTimeEditBlur(e, slot, index)}
                                                                onKeyDown={(e) => handleTimeEditKeyPress(e, slot, index)}
                                                            />
                                                            <div className="mt-1 text-[10px] text-emerald-600 font-medium">
                                                                Enter to save · Esc to cancel
                                                            </div>
