...
import React, { useEffect, useState } from "react";
import { Check, Star, Zap, ArrowRight, Phone, Route, LayoutDashboard } from "lucide-react";
...
const Pricing = () => {
    ...
    return (
        <div className="bg-slate-50">
            {/* Dashboard button — sits with a clear gap above the "Pricing" header.
                Centered on mobile, right-aligned on laptop/desktop. Always expanded, no hover needed. */}
            {dashboardHref && (
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-10 pb-8 sm:pb-10 text-center">
                <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.2em] text-blue-600 mb-3">
                    Pricing
                </p>
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-900">
...
//     return (
//         <div className="bg-slate-50">
//             {/* Dashboard button — sits in normal page flow at the very top of this
//                 section (which renders directly under the hero on the Welcome page).
//                 Centered on mobile, right-aligned on laptop/desktop. Not fixed, so it
//                 scrolls away with the page instead of sticking to the viewport top. */}
//             {dashboardHref && (
//                 <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-10">
//                     <div className="flex justify-center sm:justify-end">
//                         <Link
...
//         if (activeTab.toLowerCase() === "standard lessons" && filtered.length >= 3) {
//             // Move the "2-hour" lesson to the middle slot (Most Popular)
//             const mostPopularIndex = filtered.findIndex(
//                 (p) => p.duration && p.duration.toLowerCase().includes("2")
//             );
...
//             // Move "Mock Test" to the 3rd position (index 2)
//             const mockTestIndex = filtered.findIndex(
//                 (p) => p.duration && p.duration.toLowerCase().includes("mock test")
//             );
...
//         const isLogBookPackage = description?.toLowerCase().includes("log book package");
//         const isTestOnly = category?.toLowerCase() === "test packages" && duration?.toLowerCase().includes("test only");
//         const isMockTest = category?.toLowerCase() === "standard lessons" && duration?.toLowerCase().includes("mock test");
