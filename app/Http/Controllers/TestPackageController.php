...
public function getMonthAvailability(Request $request)
{
    ...
    while ($currentDate <= $endDate) {
        ...
        if ($currentDate->isPast() && !$currentDate->isToday()) {
            ...
            $hasSlots = $this->hasAvailableSlotsForDate($currentDate, $durationMinutes, $priceId);
            $availability[$dateKey] = [
                'has_slots' => $hasSlots,
                'is_past' => false,
                'total_slots' => $hasSlots ? $this->countAvailableSlotsForDate($currentDate, $durationMinutes, $priceId) : 0,
            ];
        }
 ...
 */
private function hasAvailableSlotsForDate($date, $durationMinutes, $priceId)
{
    $workingHoursStart = Carbon::createFromTime(7, 0, 0);
    $workingHoursEnd = Carbon::createFromTime(18, 0, 0);
    ...
    while ($currentTestTime < $workingHoursEnd) {
        ...
        if ($endTime <= $workingHoursEnd && $startTime >= $workingHoursStart) {
            $isAvailable = $this->isTestTimeAvailable(
                $date->format('Y-m-d'), 
                $startTime, 
                $endTime, 
                $priceId
 ...
 */
private function countAvailableSlotsForDate($date, $durationMinutes, $priceId)
{
    $workingHoursStart = Carbon::createFromTime(7, 0, 0);
    $workingHoursEnd = Carbon::createFromTime(18, 0, 0);
