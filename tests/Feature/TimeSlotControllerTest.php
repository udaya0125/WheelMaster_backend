...
class TimeSlotControllerTest extends TestCase
{
    ...
    public function test_custom_schedule_can_start_before_the_default_opening_time(): void
    {
        $schedules = [
            [
        ...
        foreach ($schedules as $schedule) {
            ...
            $response
                ->assertOk()
                ->assertJsonPath('success', true)
                ->assertJsonPath('updated_days', 1);

            $this->assertDatabaseCountForDate($schedule['date'], 3);
            $this->assertDatabaseHas('time_slots', [
                'date' => $schedule['date'],
    ...
    public function test_reset_keeps_seven_am_as_the_default_after_an_earlier_custom_schedule(): void
    {
        $date = now()->addDay()->toDateString();

        $this->postJson(route('ourtimeslots.update-range'), [
            'start_date' => $date,
            'end_date' => $date,
            'start_time' => '05:00',
            'end_time' => '06:00',
        ...
        $response = $this->postJson(route('ourtimeslots.reset'), [
            'date' => $date,
        ]);
    ...
    public function test_five_hour_bundle_availability_uses_selected_session_duration(): void
    {
        $date = now()->addDay()->toDateString();
        $bundle = $this->createPrice('5 Hour Lessons', '1hr or 2hr');
        $bundle->update(['category' => 'Package Bundles']);

        $oneHourResponse = $this->getJson(route('ourtimeslots.availability-summary', [
            'start_date' => $date,
            'end_date' => $date,
            'price_id' => $bundle->id,
            'duration_minutes' => 60,
        ]));
        $twoHourResponse = $this->getJson(route('ourtimeslots.availability-summary', [
            'start_date' => $date,
            'end_date' => $date,
            'price_id' => $bundle->id,
        ...
        $oneHourResponse->assertOk();
        $twoHourResponse->assertOk();
        $this->assertContains('16:40', $oneHourResponse->json("data.{$date}.available_slots"));
        $this->assertNotContains('16:40', $twoHourResponse->json("data.{$date}.available_slots"));
    }

    public function test_bundle_availability_rejects_unsupported_or_non_bundle_duration_overrides(): void
    {
        $date = now()->addDay()->toDateString();
        $bundle = $this->createPrice('5 Hour Lessons', '1hr or 2hr');
        $bundle->update(['category' => 'Package Bundles']);
        $standard = $this->createPrice('Standard lesson', '1 hour');

        $this->getJson(route('ourtimeslots.get', [
            'date' => $date,
            'price_id' => $bundle->id,
            'duration_minutes' => 90,
